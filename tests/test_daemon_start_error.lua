...
local fixture_root = repo_root .. "/tests/fixtures/failing_daemon"
...
local saw_error = vim.wait(3000, function()
  for _, message in ipairs(messages) do
    if message:find("LW: daemon failed to start: boom", 1, true) then
      return true
    end
  end
  return false
...
if not saw_error then
  error("expected daemon start failure detail; messages: " .. table.concat(messages, " | "))
end

print("daemon startup error test passed")
